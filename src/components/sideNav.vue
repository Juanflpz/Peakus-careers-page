<template>
    <div id="sidenav" class="d-flex flex-column border-rigth">
      <ul class="nav mt-2 flex-column">
        <li class="nav-item" v-for="(item, index) in navItems" :key="index">
          <a :href="item.link" class="nav-link" :class="{ active: activeIndex === index }" @click="activeIndex = index">
            <svg class="svg-icon" :xmlns="item.svgNamespace" :width="item.width" :height="item.height" :viewBox="item.viewBox">
              <g v-html="item.svgContent"></g>
            </svg>
            <span>{{ item.text }}</span>
          </a>
        </li>
...
<style scoped>
    ...
    a.active, a:hover{
        font-weight: 500;
        background-color: rgba(111, 202, 255, 0.53);
        border-radius: 5px;
    }
</style>